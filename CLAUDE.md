...
- **`backend/`** — FastAPI web API over the same `lib/` + `core/` (see `PLAN_FRONTEND.md`). `app/auth.py`: login against `APP_PASSWORD_HASH` (PBKDF2, stdlib; generate with `backend/generar_hash.py`), in-memory httpOnly-cookie sessions, 5/min login rate-limit — the router password never reaches the browser. `app/deps.py`: `get_api` dependency yields ONE persistent shared router connection under a **global lock** (a `!trap` keeps it alive, a network error resets it, idle >60s triggers a cheap liveness check; per-request connections would flood the router syslog with login/logout lines). `app/routers/` mirrors `core/` sections; all routes require the session dependency except `/api/auth/*` and `/api/salud` (healthcheck). Write endpoints (bloqueos, horario, qos deploy/reset) additionally require `{"confirmar": true}` in the body — the API-side mirror of the menu's `CONFIRMAR` dict; without it they return 400 and touch nothing. `app/ws.py`: `/ws/monitor`, `/ws/log` and `/ws/qos` — one shared sampling loop per stream (`Muestreador`): first client starts it, last stops it, all get the same snapshot; sampling goes through `deps.usar_api` so HTTP and WS share the SAME persistent connection (navigating pages must not produce router logins); invalid session closes with code 4401; sampling interval via `APP_WS_INTERVALO` (tests set it tiny and monkeypatch `deps.MikroTikAPI` to inject the FakeAPI). Exceptions map like CLI exit codes: `MikroTikConnectionError`/`OSError` → 502, `MikroTikCommandError` → 400, Spanish detail + `sugerencia`. Hardening: session cookie is httpOnly + SameSite=Strict; a middleware in `main.py` adds security headers (nosniff, DENY, no-referrer, no-store) to every API response, and nginx adds CSP + friends for the SPA. Tests in `backend/tests/` (pytest + FakeAPI injected via `dependency_overrides`; venv at `backend/.venv`, gitignored).
...
- **`docker-compose.yml`** — 2 services: `api` (no published port) and `web` (single exposed port, `PANEL_PORT` in `.env`, default 8080). Both run non-root (`web` uses `nginx-unprivileged` listening on 8080 inside) with `no-new-privileges`. `config.env` is passed via `env_file` (includes `APP_PASSWORD_HASH`); `TZ` (from `.env`) sets snapshot timestamps; `config/` and `backups/` are volumes shared with the CLI. Backend Dockerfile builds from the **repo root** context (needs `lib/` + `core/`). CI lives in `.github/workflows/ci.yml` (unittest, pytest, frontend build+vitest, compose build).
